  ...
  describe '#timetable' do
    context 'when invalid route' do
      it 'raises an error' do
        expect { transport_nsw.timetable('unknown_route', '2014-03-18') }.to raise_error(ArgumentError)
      end
    end

    context 'when invalid date' do
      it 'raises an error' do
        expect { transport_nsw.timetable('To Manly:Slow Ferry', 'invalid_date') }.to raise_error(ArgumentError)
      end
    end
